...
void Server::createEpoll() {
    ...
    if (epoll_fd_ == -1) {
        throw HttpServerException{"Epoll create failed"};
    }
...
void Server::epollEvents(const file_descriptor::Descriptor& fd, const uint32_t events, const EpollCtlOptions option) {
    ...
    if (epoll_ctl(epoll_fd_, int(option), fd, &event) == -1) {
        throw HttpServerException{"epoll_ctl add failed"};
    }
}
...
void Server::acceptClients() {
    ...
    while (true) {
        ...
        if (fd == -1) {
            ...
            throw HttpServerException{"accept4 error"};
        }
...
void Server::eventLoop(const size_t epoll_size) {
    ...
    while (true) {
        ...
        if (ndfs == -1) {
            ...
            throw HttpServerException{"epoll_wait error"};
        }
...
void routine1(size_t init, size_t max) {
	coroutine::routine_t id = coroutine::current();
...
void Server::handleClient(const int fd, const uint32_t events) {
    ...
    coroutine::routine_t c_id = coroutine::current();
	std::cout << "c_id = " << c_id << std::endl;
    std::cout << "routine1" << std::endl;
	coroutine::routine_t id = coroutine::create(routine1, 1, 10);
